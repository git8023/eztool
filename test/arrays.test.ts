...
test('arrays.remove', () => {
  ...
  expect(arrays.remove(arr, {id: 3}, 'id')).toEqual({
    id: 3,
    foo: 'foo'
  })
  expect(arrays.remove(arr, null, (el, i) => i === el)).toBeNull()
  expect(arrays.remove<any>(arr, null, el => 'bar' === el.bar)).toEqual({
    id: 4,
    bar: 'bar'
  })
})
...
test('arrays.removeAll', () => {
  ...
  expect(arrays.removeAll<any>(arrD, arrC)).toEqual([{id: 2}, {
    id: 2,
    bar: 'bar'
  }])
})
